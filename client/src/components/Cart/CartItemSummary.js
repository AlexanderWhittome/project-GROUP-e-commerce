  ...
  return (
    <Row>
     ...
     <Row>
        ...
        <DeleteBlock>
          {!deleteDialogVisible && (
            <GenericButton onClick={()=>setDeleteDialogVisible((state)=>!state)}>Remove from cart</GenericButton>
          )}
          {deleteDialogVisible && [
            <GenericButton>{"\u2713"}</GenericButton>,
            <GenericButton onClick={()=>setDeleteDialogVisible((state)=>!state)}>{"\u2717"}</GenericButton>,
          ]}
